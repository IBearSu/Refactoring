...
// Возвращаем ссылку на список аккаунтов клиента (без возможности изменения извне)
const std::list<int>& Client::get_client_account_IDs() const {
    return client_account_IDs_;
}

// Создать новый аккаунт (используем внутренний account_manager_)
...
// Удалить аккаунт (используем внутренний account_manager_)
